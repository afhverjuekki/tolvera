...
class CV:
    def __init__(self, tolvera, **kwargs) -> None:
        ...
        self.rgba = ti.Vector.field(4, dtype=ti.f32, shape=(self.x, self.y))
    ...
    @ti.kernel
    def frame2rgba(self, f: ti.types.ndarray(dtype=ti.f32, ndim=3)):
        for i, j in ti.ndrange(self.x, self.y):
            _i, _j = self.y - j, self.x - i
            r = f[_i, _j, 2] / 255
            g = f[_i, _j, 1] / 255
            b = f[_i, _j, 0] / 255
            self.rgba[i, j] = [r, g, b, 1]
    ...
    @ti.kernel
    def img2rgba(self, img: ti.types.ndarray(dtype=ti.f32, ndim=2)):
        for i, j in ti.ndrange(self.x, self.y):
            _i, _j = self.y - j, self.x - i
            p = img[_i, _j] / 255
            self.rgba[i, j] = [p, p, p, 1]
    ...
    def process(self):
        ...
        if self.i % self.camera_substeps == 0:
            ...
            if self.colormode == "rgba":
                # print(f"[{self.tv.name}] Processing camera frame...")
                # self.frame2rgba(self.camera_frame)
                self.frame2rgba(self.camera_frame)
                # self.px.px.rgba = self.rgba
