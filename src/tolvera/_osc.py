...
class OSC:
    ...
    def init_osc(self, **kwargs):
        ...
        print(f"[Tölvera.OSC] Initialising OSC host '{self.host_ip}:{self.receive_port}'.")
        self.host = iiOSC(self.host_ip, self.receive_port, verbose=self.verbose, concurrent=True)
        print(f"[Tölvera.OSC] Initialising OSC client '{self.client_name}' at '{self.client_ip}:{self.send_port}'.")
