...
__all__ = ['move', 'attract', 'attract_species', 'repel', 'repel_species', 'gravitate', 'gravitate_species']

@ti.kernel
def move(particles: ti.template()):
    for i in range(particles.field.shape[0]):
        if particles.field[i].active == 0: continue
...
@ti.kernel
def attract_species(particles: ti.template(), pos: ti.math.vec2, mass: ti.f32, radius: ti.f32, species: ti.i32):
    for i in range(particles.field.shape[0]):
        p = particles.field[i]
        if p.active == 0: continue
...
def gravitate(particles: ti.template(), G: ti.f32, radius: ti.f32):
    for i, j in ti.ndrange(particles.field.shape[0], particles.field.shape[0]):
        ...
        if (p2.pos - p1.pos).norm() > radius: continue
        particles.field[i].vel += gravitation(p1, p2, G)

@ti.kernel
def gravitate_species(particles: ti.template(), G: ti.f32, radius: ti.f32, species: ti.i32):
    for i, j in ti.ndrange(particles.field.shape[0], particles.field.shape[0]):
        if i == j: continue
        p1 = particles.field[i]
        p2 = particles.field[j]
        if p1.species != species or p2.species != species: continue
        if (p2.pos - p1.pos).norm() > radius: continue
        particles.field[i].vel += gravitation(p1, p2, G)
