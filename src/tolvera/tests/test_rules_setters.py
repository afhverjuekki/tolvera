...
from tolvera import Rules, Tolvera
...
def test_set_all_ndarray(tolvera_setup):
    tv = tolvera_setup
    rules_all_ndarray = np.zeros(
        (tv.o.species, tv.o.species, len(tv.flock.rules.dict)), dtype=np.float32
    )
...
def test_set_species_ndarray(tolvera_setup):
    tv = tolvera_setup
    rules_species_ndarray = np.zeros(
        (tv.o.species, len(tv.flock.rules.dict)), dtype=np.float32
    )
