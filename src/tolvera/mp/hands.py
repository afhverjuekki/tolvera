...
class MPHands:
    def __init__(self, context, **kwargs) -> None:
        ...
        self.config = {
            ...
            "min_tracking_confidence": kwargs.get("min_track_con", 0.5),
        }
        ...
        self.hands_np = {
            "pxnorm": np.zeros(
                (self.config["max_num_hands"], self.n_conns, 3), np.float32
            ),
    ...
    def setup_connections(self):
        ...
        self.index_conns = HandConnection.field(
            shape=(len(HAND_INDEX_FINGER_CONNECTIONS))
        )
        self.middle_conns = HandConnection.field(
            shape=(len(HAND_MIDDLE_FINGER_CONNECTIONS))
        )
        self.ring_conns = HandConnection.field(
            shape=(len(HAND_RING_FINGER_CONNECTIONS))
        )
        self.pinky_conns = HandConnection.field(
            shape=(len(HAND_PINKY_FINGER_CONNECTIONS))
        )
