...
class MPFaceMesh:
    def __init__(self, context, **kwargs) -> None:
        ...
        self.config = {
            ...
            "min_tracking_confidence": kwargs.get("tracking_con", 0.5),
        ...
        self.face_mesh_np = {
            "pxnorm": np.zeros(
                (self.config["max_num_faces"], self.n_points, 3), np.float32
            ),
            "px": np.zeros(
                (self.config["max_num_faces"], self.n_points, 2), np.float32
            ),
        }
    ...
    def setup_connections(self):
        ...
        self.left_eyebrow_conns = FaceMeshConnection.field(
            shape=(len(FACEMESH_LEFT_EYEBROW))
        )
        self.right_eye_conns = FaceMeshConnection.field(shape=(len(FACEMESH_RIGHT_EYE)))
        self.right_eyebrow_conns = FaceMeshConnection.field(
            shape=(len(FACEMESH_RIGHT_EYEBROW))
        )
        self.right_iris_conns = FaceMeshConnection.field(
            shape=(len(FACEMESH_RIGHT_IRIS))
        )
